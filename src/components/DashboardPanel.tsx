    ...
    <aside className="dashboard-panel" style={{ left: position.left, top: position.top }} aria-label="Grid stability dashboard">
      ...
      {selectedPlant && (
        <section className="location-summary">
          ...
          <div className="site-primary-metrics"><div><span>Retirement</span><strong>{retirementLabel(selectedPlant)}</strong></div><div><span>Confidence</span><strong>{selectedPlant.confidenceScore ? `${selectedPlant.confidenceScore}/100` : 'Not scored'}</strong></div></div>
          ...
          <div className="site-context"><div><span>By {year}</span><strong>{formatMw(locationRetiringMw)} at location</strong></div><div><span>Share of local capacity</span><strong>{selectedShare}%</strong></div><div><span>Next at location</span><strong>{locationNextRetirement ? `${locationNextRetirement.name} · ${effectiveRetirementYear(locationNextRetirement)}` : 'Unconfirmed'}</strong></div></div>
