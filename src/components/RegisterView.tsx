...
function PlantEditor({ portfolio, initialPlant, plants, onClose, onSave }: { portfolio: PortfolioId, initialPlant: Plant, plants: Plant[], onClose: () => void, onSave: (plant: Plant) => void }) {
  ...
  const [locationQuery, setLocationQuery] = useState(() => initialPlant.nodeName || initialPlant.name)
  ...
  useEffect(() => {
    const nextPlant = prepareForEditing(initialPlant, portfolio)
    setPlant(nextPlant)
    setLocationQuery(nextPlant.nodeName || nextPlant.name)
    setSelectedLocation(nextPlant.hasCoordinates ? `${nextPlant.latitude.toFixed(5)}, ${nextPlant.longitude.toFixed(5)}` : '')
  }, [initialPlant, portfolio])
  ...
  const lookupLocation = async () => {
    ...
    if (query.length < 2) { setLookupError('Enter at least two characters to search for a site, substation, town, or postcode.'); return }
    ...
    try {
      ...
      if (!results.length && plant.country) results = await search(`${query}, ${plant.country}`)
      setLocationResults(results)
      if (!results.length) setLookupError('No locations matched. Try a broader place name, nearby town, or postcode.')
