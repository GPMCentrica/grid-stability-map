...
export function DataQualityView({ plants, onEdit }: { plants: Plant[], onEdit: (plant: Plant) => void }) {
  const issues: Issue[] = plants.flatMap((plant) => {
    ...
    if (!plant.retirementDate) recordIssues.push({ type: 'Missing retirement date', detail: 'Confirm the best available retirement date.', plant })
  ...
  const grouped = ['Missing retirement date', 'Missing location', 'Low confidence', 'Duplicate plant name', 'Inconsistent location name'].map((type) => ({ type, items: issues.filter((issue) => issue.type === type) })).filter((group) => group.items.length)
  const icons = { 'Missing retirement date': CalendarDays, 'Missing location': MapPin, 'Low confidence': AlertCircle, 'Duplicate plant name': SearchCheck, 'Inconsistent location name': SearchCheck }
