...
- Each node compares the assumed technical provision retiring by the selected horizon against total assumed provision in the local plant register. The compact node marker represents absolute assumed provision lost; its colour represents the local share lost.
...
- A translucent footprint ring communicates the assumed spatial dependency of each service. SCL uses a 25–50 km range, Voltage uses 50–100 km, and Inertia uses 100–250 km; a higher local loss share moves the node towards the upper end of its layer's range. These ranges are visual screening assumptions, not electrical reach calculations.
